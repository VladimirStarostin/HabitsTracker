...
let defaultEmail = "test@email.com"
let defaultPassword = "p@$$w0rd"
let defaultName = "Bob"

[<SetUp>]
let Setup () =
    task {
        let! _ = UsersUtils.deleteAllAsync |> Postgres.executeWithConnection UsersCommon.connectionStringForTests
        return ()
    }
...
let Test1 () =
    task {
        ...
        let! newUser = Auth.register client { Auth.RegisterParameters.Email = defaultEmail; Password = defaultPassword; Name = "Bob" }

        let! resp =
            Auth.login
                client
                { Auth.LoginParameters.Email = defaultEmail
