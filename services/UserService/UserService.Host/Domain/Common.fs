                ...
                then
                    ...
                    try
                        ...
                        if parts.Length = 4 && parts[0] = "pbkdf2-sha256" then
                            ...
                            if Internal.constantTimeEquals want got then
                                Ok ()
                            else
...
module Operations =
    ...
    let saveNewTokenAsync<'err> (newToken: RefreshToken) (conn: IDbConnection) : Task<PipelineResult<string, 'err>> =
        task {
            let! _ =
                RefreshTokens.insertSingleAsync
    ...
    let revokeOldToken<'err> (token: RefreshToken) (conn: IDbConnection) : Task<PipelineResult<unit, 'err>> =
        RefreshTokens.revokeByTokenAsync token.Token conn
        |> Task.map (fun _ -> Ok ())

    let revokeOldTokenForUser<'err> (userId: int) (conn: IDbConnection) : Task<PipelineResult<unit, 'err>> =
        RefreshTokens.revokeByUserIdAsync userId conn
        |> Task.map (fun _ -> Ok ())
