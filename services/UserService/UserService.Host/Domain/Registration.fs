...
type RegistrationPipelineResult<'Ok> = PipelineResult<'Ok, RegistrationError>

module private Steps =
    let ensureUserNotExists (email: string) (conn: IDbConnection) : Task<RegistrationPipelineResult<unit>> =
        task {
            try
    ...
    let hashPassword (plainPassword: string) : RegistrationPipelineResult<string> =
        try
    ...
    let saveNewUserAsync
        ...
        (conn: IDbConnection)
        : Task<RegistrationPipelineResult<User>> =
        task {
            try
                let! newUser =
                    insertSingleAsync
...
let registerAsync
    ...
    (conn: IDbConnection)
    : Task<RegistrationPipelineResult<AuthResponse>> =
    registration {
