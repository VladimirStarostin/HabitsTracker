...
module internal Steps =
    let getEventsByHabitIdAsync (habitId: int) (trackingClient: IHabitsTrackingClient) : Task<DeleteHabitPipelineResult<HabitEventDto list>> =
        task {
    ...
    let deleteEventsAsync
        ...
        task {
            for ev in events do
                let! _ = trackingClient.DeleteAsync ev.Id userId
                ()

            return DeleteHabitPipelineResult.Ok ()
        }

    let deleteHabitAsync (habitId: int) (habitsClient: IHabitsClient) : Task<DeleteHabitPipelineResult<int>> =
        task {
            let! deleted = habitsClient.DeleteAsync habitId
            return
                if deleted = 1 then
...
let tryDeleteHabitAsync
    ...
    deleteHabitPipeline {
        ...
        return! Steps.deleteHabitAsync habitId habitsClient
    }
