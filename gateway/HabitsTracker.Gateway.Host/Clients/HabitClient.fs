...
type IHabitsClient =
    abstract GetByUserIdsAsync: int Set -> Task<HabitDto Set>
    abstract GetByIdsAsync: int Set -> Task<HabitDto Set>
...
type HabitsClient (grpcClient: HabitsService.HabitsServiceClient) =
    ...
    interface IHabitsClient with
        ...
        member this.GetByIdsAsync (habitIds: Set<int>) : Task<HabitDto Set> =
            task {
                let request = GetByIdsRequest ()
                request.HabitIds.AddRange habitIds
                let! response = grpcClient.GetByIdsAsync request
                return response.Habits |> Seq.map toHabitDto |> Set.ofSeq
            }

        member this.GetByUserIdsAsync (userIds: Set<int>) : Task<HabitDto Set> =
            task {
