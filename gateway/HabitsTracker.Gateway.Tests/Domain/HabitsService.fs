    ...
    task {
        let habitsClient =
            { new IHabitsClient with
                member _.DeleteAsync _ = task { return 0 }
        ...
        match! Steps.deleteHabitAsync 1 habitsClient with
...
let ``tryDeleteHabitAsync - removes events and habit`` () =
    task {
        let events =
            ref
                [ { Id = 1; HabitId = 1; Date = DateTimeOffset.UtcNow }
                  { Id = 2; HabitId = 1; Date = DateTimeOffset.UtcNow } ]

        let trackingClient =
            { new IHabitsTrackingClient with
                member _.GetByHabitIdsAsync ids = task { return !events |> List.filter (fun e -> List.contains e.HabitId ids) }
                member _.GetByIdsAsync ids = task { return !events |> List.filter (fun e -> List.contains e.Id ids) }
                member _.TrackHabitAsync _ _ = task { return Unchecked.defaultof<_> }
                member _.DeleteAsync id _ = task { events := List.filter (fun e -> e.Id <> id) !events; return 1 } }

        let habitDeleted = ref false
        let habitsClient =
            { new IHabitsClient with
                member _.DeleteAsync _ = task { habitDeleted := true; return 1 }
                member _.CreateAsync _ = task { return Unchecked.defaultof<_> }
                member _.GetByIdsAsync _ = task { return [] }
                member _.GetByUserIdsAsync _ = task { return [] } }
