...
type TestApiResponse<'a> = Task<Result<'a, HttpProblem>>
...
let readAs<'T> (resp: HttpResponseMessage) : TestApiResponse<'T> =
    task {
...
module Auth =
    ...
    let register (client: HttpClient) (input: Api.RegisterParameters) : TestApiResponse<Api.AuthResponse> =
        client.PostAsJsonAsync ("api/v1/auth/register", input)
        |> Task.mapAsync readAs

    let login (client: HttpClient) (input: Api.LoginParameters) : TestApiResponse<Api.AuthResponse> =
        client.PostAsJsonAsync ("api/v1/auth/login", input)
        |> Task.mapAsync readAs

    let refresh (client: HttpClient) (input: Api.RefreshTokenParameters) : TestApiResponse<Api.AuthResponse> =
...
module Habits =
    ...
    let createHabit
        ...
        : TestApiResponse<Api.Habit> =
        client.DefaultRequestHeaders.Authorization <- AuthenticationHeaderValue ("Bearer", token)

        client.PostAsJsonAsync ("api/v1/habits", input)
        |> Task.mapAsync readAs

    let getCurrentUserHabits (client: HttpClient) (token: string) : TestApiResponse<Api.Habit list> =
        client.DefaultRequestHeaders.Authorization <- AuthenticationHeaderValue ("Bearer", token)

        client.GetFromJsonAsync "api/v1/habits"
        |> Task.mapAsync readAs

    let getById (client: HttpClient) (token: string) (habitId: int) : TestApiResponse<Api.Habit option> =
        client.DefaultRequestHeaders.Authorization <- AuthenticationHeaderValue ("Bearer", token)

        client.GetAsync $"api/v1/habits/{habitId}"
        |> Task.mapAsync readAs

    let deleteHabitById (client: HttpClient) (token: string) (habitId: int) : TestApiResponse<int> =
        client.DefaultRequestHeaders.Authorization <- AuthenticationHeaderValue ("Bearer", token)
...
module Events =
    ...
    let getEventsByHabitId
        ...
        (token: string)
        (habitId: int)
        : TestApiResponse<Api.HabitEventInfo list> =
        client.DefaultRequestHeaders.Authorization <- AuthenticationHeaderValue ("Bearer", token)

        client.GetAsync $"api/v1/events/by-habit/{habitId}"
        |> Task.mapAsync readAs

    let getEventById (client: HttpClient) (token: string) (eventId: int) : TestApiResponse<Api.HabitEventInfo> =
        client.DefaultRequestHeaders.Authorization <- AuthenticationHeaderValue ("Bearer", token)

        client.GetAsync $"api/v1/events/{eventId}"
        |> Task.mapAsync readAs

    let deleteEvent (client: HttpClient) (token: string) (eventId: int) : TestApiResponse<int> =
        client.DefaultRequestHeaders.Authorization <- AuthenticationHeaderValue ("Bearer", token)
    ...
    let createEvent
        ...
        : TestApiResponse<Api.HabitEventInfo> =
        client.DefaultRequestHeaders.Authorization <- AuthenticationHeaderValue ("Bearer", token)
...
module Users =
    let deleteUser (client: HttpClient) (token: string) (userId: int) : TestApiResponse<int> =
        client.DefaultRequestHeaders.Authorization <- AuthenticationHeaderValue ("Bearer", token)
