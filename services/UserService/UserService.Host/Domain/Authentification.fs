...
type AuthenticationPipelineResult<'Ok> = PipelineResult<'Ok, AuthError>

module internal Steps =
    let validateInput (requestedToken: string) : AuthenticationPipelineResult<string> =
        if String.IsNullOrWhiteSpace requestedToken then
            Error MissingToken
        else
            AuthenticationPipelineResult.Ok requestedToken

    let tryFindExistingInDbAsync
        (okToken: string)
        (conn: IDbConnection)
        : Task<AuthenticationPipelineResult<RefreshToken>> =
    ...
    let validateExistingRefreshToken (token: RefreshToken) : AuthenticationPipelineResult<RefreshToken> =
    ...
    let revokeOld (token: RefreshToken) (conn: IDbConnection) : Task<AuthenticationPipelineResult<unit>> =
        revokeByTokenAsync token.Token conn
        |> Task.map (fun _ -> Ok ())

let internal validation = PipelineBuilder ()

let validateAndRefreshAsync
    (initialToken: string)
    (jwtSettings: JwtSettings)
    (conn: IDbConnection)
    : Task<AuthenticationPipelineResult<AuthResponse>> =
    validation {
